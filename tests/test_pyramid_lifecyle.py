...
async def test_pc() -> None:
    async with sim_and_client() as (sim, client):
        ...
        SEED: bytes = bytes([0,  51, 6,  244, 24,  199, 1,  25,  52,  88,  129,
        ...
        PC_MOD = load_clvm(PC_CLSP, package_or_requirement="chialisp")
        PC = PC_MOD.curry(FAKE_PUBLIC_KEY) 
        ...
        SOLUTION = Program.to([MEMO,puzzle_hashes,PAYOUT_AMOUNT,PC_FEE])
        cds = PC.run(SOLUTION) #conditions
